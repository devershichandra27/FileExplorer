/*Improved acceptcommand.cpp. Switching to normal on esc key*/
#include <unistd.h>
#include <fcntl.h>
#include <dirent.h>
#include <dirent.h>
...
void copydir(string sourcedirectory, string destinationdirectory)
{
    string tmd = destinationdirectory + "/" + sourcedirectory;
    int m = mkdir(tmd.c_str(), S_IRWXU | S_IRWXG | S_IRWXO);
    DIR *dp = opendir(sourcedirectory.c_str());
    struct dirent *entry;
    if (dp)
    {
        while ((entry = readdir(dp)) != NULL)
        {
            ...
            {
                string fullpath = sourcedirectory + "/" + fname;
                cout << "File: fullpath = " << fullpath << endl;
                struct stat fs;
                stat(fullpath.c_str(), &fs);
                if (S_ISDIR(fs.st_mode))
                {
                    copydir(fullpath, destinationdirectory);
                }
                else
                {
                    vector<string> fv;
                    fv.push_back(fullpath);
                    copyutil(destinationdirectory, fv);
                    fv.clear();
                }
            }
        }
    ...
    struct stat t;
    stat(sourcedirectory.c_str(), &t);
    chmod(tmd.c_str(), t.st_mode);
}
...
void copydirutil(vector<string> s, string dest)
{
    for (auto source : s)
    {
        copydir(source, dest);
...
void movedirutil(vector<string> s, string dest)
{
    for (string source : s)
    {
        copydir(source, dest);
        char *k = new char[256];
        strcpy(k, source.c_str());
        removedir(k);
    }
}
...
void splitcommand(string s)
{
    ...
    vector<string> filelist(vi.begin() + 1, vi.end() - 1); //will store the source file/folder list.
    ...
    else if (strcmp(command.c_str(), "copy_dir") == 0)
    {
        copydirutil(filelist, destination);
    }
    else if (strcmp(command.c_str(), "move") == 0)
    {
        moveutil(destination, filelist);
    }
    else if (strcmp(command.c_str(), "move_dir") == 0)
    {
        movedirutil(filelist, destination);
    }
    ...
    else if (strcmp(command.c_str(), "delete_file") == 0)
    {
        string file = vi[1];
        remove(file.c_str());
    }
    else if (strcmp(command.c_str(), "delete_dir") == 0)
    {
        string file = vi[1];
        char *f = new char[256];
        strcpy(f, file.c_str());
